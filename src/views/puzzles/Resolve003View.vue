<template>
  <div id="puzzleResolver">
    ...
    <div class="search-controls">
      ...
      <!-- AGREGAR ESTE BLOQUE PARA EL CONTROL DE VELOCIDAD -->
      <div class="input-group">
        <label for="intervalDelay">Velocidad de Búsqueda (ms de retraso, menor es más rápido):</label>
        <input 
          type="range" 
          id="intervalDelay" 
          v-model.number="intervalDelay" 
          min="0" 
          max="100" 
          step="1"
        >
        <span>{{ intervalDelay }} ms</span>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      targetAddress: '1Dop1pfHykey7otiHbZbutGFAc1foJZo12',
      // Estado de búsqueda
      isSearching: false,
      isPaused: false,
      solutionFound: false,
      ...
      solutionHtml: '',
      // AGREGAR intervalDelay PARA CONTROLAR LA VELOCIDAD
      intervalDelay: 10,
  ...
  methods: {
    ...
    startSearch() {
      ...
      // Iniciar intervalos
      // MODIFICAR PARA USAR intervalDelay EN LUGAR DEL VALOR FIJO 10
      this.searchInterval = setInterval(() => {
        if (this.isPaused) return;
        this.searchStep();
      }, this.intervalDelay); // Ejecutar paso de búsqueda cada this.intervalDelay ms
