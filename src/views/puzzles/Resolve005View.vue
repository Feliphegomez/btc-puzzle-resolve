<template>
  <div id="puzzleResolver">
    ...
    <div class="search-controls">
      ...
      <div class="input-group">
        <label for="speedMode">Velocidad de Búsqueda:</label>
        <select id="speedMode" v-model="speedMode" :disabled="isSearching">
          <option value="slow">🐌 Lenta (100ms delay) - Menor uso de CPU</option>
          <option value="normal">⚡ Normal (10ms delay) - Balanceado</option>
          <option value="fast">🚀 Rápida (1ms delay) - Mayor rendimiento</option>
          <option value="turbo">💨 Turbo (sin delay) - Máximo rendimiento</option>
        </select>
      </div>
      ...
      <div class="input-group">
        ...
        <select id="batchSize" v-model="batchSize" :disabled="isSearching">
          <option value="1">1 clave por ciclo (más estable)</option>
          <option value="10">10 claves por ciclo (balanceado)</option>
          <option value="50">50 claves por ciclo (más rápido)</option>
          <option value="100">100 claves por ciclo (máxima velocidad)</option>
        </select>
      </div>
    ...
    <div class="status" v-if="isSearching || isPaused">
      ...
      <div class="speed-info">
        <p><strong>Configuración:</strong> {{ getSpeedDescription() }}</p>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      targetAddress: '1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa', // SN
  ...
  methods: {
    ...
    if (this.isPaused) this.stopSearch();
      ...
      this.addLog(`⚙️ Configuración: ${this.getSpeedDescription()}`);
    ...
    async searchStep() {
      ...
      // Procesar múltiples claves según el batchSize configurado
      ...
      for (let i = 0; i < batchCount; i++) {
        ...
        // --- NUEVO: Consultar saldo ---
        const balance = await this.checkBalance(walletInfo.address);
        console.log('balance', balance)
        if (balance > 0) {
            console.log("🎉 ¡Billetera con saldo encontrada! 🎉", {
            address: walletInfo.address,
            balanceSatoshis: balance,
            // balanceBTC: balance / 100000000, // Descomentar para ver en BTC
            privateKeyHex: walletInfo.privateKeyHex,
            privateKeyWIF: walletInfo.privateKey
            });
            // Opcional: también puedes agregarlo al log del componente
            this.addLog(`🎉 Saldo encontrado: ${walletInfo.address} (${balance} satoshis)`);
        }
        // --- FIN NUEVO ---
    ...
    async checkBalance(address) {
        ...
        try {
            ...
            if (!response.ok) {
            // Si la dirección no tiene actividad, la API puede devolver 404 u otro error
            if (response.status === 404) {
                // console.log(`La dirección ${address} no tiene actividad.`);
                return 0; // Considerar saldo 0 si no hay datos
            }
            ...
            // Extraer el saldo final en satoshis
            const balanceSatoshis = data.final_balance; // Puede ser 0
            
            // Convertir satoshis a BTC (opcional, para mostrar)
            // const balanceBTC = balanceSatoshis / 100000000;
            
            return balanceSatoshis;
        } catch (error) {
            console.error(`Error al obtener el saldo para ${address}:`, error);
            // Dependiendo de cómo quieras manejar errores de red o API,
            // podrías devolver 0 o simplemente registrar el error.
            return 0; // Por ejemplo, asumir 0 si hay error
        }
    },
  }
};
